    ...
    , ui(new Ui::CheckTable)
{
    ...
    ui->videoStatus->setCurrentIndex(0);
    // 此处使用正则表达式对视频用户id进行校验
    // 视频用户id是服务端生成，格式为：7d98-a3846253-0003
    // QRegularExpression 是Qt中用于处理正则表达式的类
    QRegularExpression regExp("^[0-9a-f]{4}-[0-9a-f]{8}-[0-9a-f]{4}$");
    QValidator* validator = new QRegularExpressionValidator(regExp, this);
    // 将创建的验证器设置到ui->userIdEdit上
    // 用户在 userIdEdit 中输入的内容将被验证是否符合正则表达式定义的格式
    ui->userldEdit->setValidator(validator);
    // 创建分页器并显示
